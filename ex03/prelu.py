...
# Full PRelu
# not working, row vector get broadcastet to matrix -> mismatch in T.grad
def init_slope(shape):
    return theano.shared(floatX(0.25 + np.zeros((1,shape))),
                        broadcastable=(True,False))
# layer wise PRelu
def init_slope(a):
    return theano.shared(floatX(a))
...
for i in range(100): #you can adjust this if training takes too long
    ...
    print("correct: {0:.4f}, a: {1:.2f}, a2: {0:.2f}".format(
               error, float(s_h.get_value()), float(s_h2.get_value())) )
