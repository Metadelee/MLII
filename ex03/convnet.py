...
def convlayer(X, pic_in, image_shape, filter_shape, first=False):
    if first:
        conv = conv2d(X, pic_in, image_shape, filter_shape, border_mode='half') #half perfoms as scipy.signals 'full' for odd filter sizes
    else:
        conv = conv2d(X, pic_in, image_shape, filter_shape)
    max_pool = pool_2d(conv, ds=(2,2), mode='max')
...
trX = trX.reshape(-1, 1, 28, 28) # -1 indicates broadcastable dim
trY = trY.reshape(-1, 1, 28, 28)
